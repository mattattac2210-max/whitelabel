...
- **Estimate Mode**: Two modes — "Industry Averages" (default ON, fast setup, uses typical running costs by vehicle category) and "Personalised" (requires detailed inputs). Toggle in Settings with confirmation modal when switching to personalised. Industry mode uses INDUSTRY_RUNNING_COSTS by vehicle category (ute-4x4: $12,900, etc.) plus estimated depreciation. `getEstimateMode()` returns 'industry' or 'personalised'. In industry mode, deduction state is always 'partial' with asterisk. `localStorage.wc_settings.useIndustryAverages` (default true).
