...
- **Deduction Estimator**: Conditional system with 3 states (LOCKED / PARTIAL / ACTIVE). Logic in `client/src/lib/deduction-estimator.ts`. Readiness checks: tax profile complete, vehicle purchase details, WDV entered, purchase price entered, business use established (trips classified), expenses entered. Dashboard, Sort, and Tax Estimate screens all respect the estimator state. Settings toggle `showDeductionEstimates` controls visibility. Locked modal explains what's missing with navigation CTAs. Partial shows values with asterisk. Active shows normally. Enhanced depreciation supports WDV, dateFirstUsed, previouslyClaimed.
