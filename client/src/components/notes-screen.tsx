...
export function NotesScreen() {
  ...
  const allHaveNotes = bizTrips.every(({ idx }) => {
    const text = noteTexts[idx] ?? state.trips[idx]?.notes ?? '';
    return text.trim().length > 0;
  });
  const withNotes = bizTrips.filter(({ idx }) => (noteTexts[idx] ?? '').trim().length > 0).length;
