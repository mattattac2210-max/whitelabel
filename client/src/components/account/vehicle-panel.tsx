...
import { Car, Search, AlertTriangle, Info } from 'lucide-react';
  ...
  return (
    <CollapsiblePanel title="Vehicle Details" icon={Car} testId="panel-vehicle">
      <div className="pt-[12px]">
        ...
        {/* ─── Card 1: Purchase, Ownership & Finance ─── */}
        <div className="rounded-[12px] p-[14px] mt-[10px]" style={{ background: 'rgba(255,255,255,.015)', border: '1px solid rgba(255,255,255,.06)' }}>
          ...
          <div className="grid grid-cols-3 gap-[6px] mt-[10px]">
            <StatCell label="Fuel Economy" value={fuelEconomy > 0 ? `${fuelEconomy}L` : '—'} color="var(--wc-y)" />
            <StatCell label="Est. Running" value={estAnnualRunning > 0 ? `$${estAnnualRunning.toLocaleString()}` : '—'} color="var(--wc-y)" />
        ...
        <div className="rounded-[12px] p-[14px] mt-[10px]" style={{ background: 'rgba(255,255,255,.015)', border: '1px solid rgba(255,255,255,.06)' }}>
          <div className="font-heading font-black text-[13px] uppercase tracking-[.05em] text-white mb-[10px]">Depreciation Details</div>
          ...
          {!depCalc && rawPrice <= 0 && (
            <div className="text-[11px] py-[8px]" style={{ color: 'var(--wc-t3)' }}>
              Enter a purchase price above to see depreciation estimates.
            </div>
          )}
        </div>
      </div>
