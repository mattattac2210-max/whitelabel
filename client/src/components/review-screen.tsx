          ...
          {sorted.map((t) => {
            ...
            return (
              <div
                ...
                style={{
                  background: t.autoGenerated ? 'rgba(245,158,11,.03)' : 'var(--wc-card)',
                  border: t.autoGenerated ? '1px dashed rgba(245,158,11,.3)' : '1px solid var(--wc-border)',
                  borderLeft: t.autoGenerated ? '3px dashed rgba(245,158,11,.5)' : t.type === 'business' ? '3px solid rgba(245,196,0,.6)' : '3px solid rgba(180,180,180,.25)',
                }}
              ...
              >
                <div className="flex items-center gap-[10px] p-[12px_14px]" onClick={() => setExpandedTrip(isExpanded ? null : origIdx)}>
                  <div className="flex-1 min-w-0">
                    <div className={`font-semibold text-[13px] mb-[3px] ${isExpanded ? '' : 'truncate'}`} style={{ color: t.autoGenerated ? 'var(--wc-am)' : 'white' }}>
                      {t.autoGenerated && <span className="font-data text-[8px] uppercase tracking-[.08em] mr-[5px] px-[4px] py-[1px] rounded-[3px]" style={{ background: 'rgba(245,158,11,.12)', color: 'var(--wc-am)', border: '1px solid rgba(245,158,11,.2)' }}>GAP</span>}
                      {t.from} &rarr; {t.to}
                    </div>
                    <div className="text-[11px]" style={{ color: 'var(--wc-t3)' }}>
                      {t.autoGenerated ? 'Unlogged gap — connects odometer chain' : `${t.date} · ${t.km} km · ${t.duration}`}
                    </div>
                  </div>
                  <div className="font-heading font-bold text-[14px] flex-shrink-0" style={{ color: t.autoGenerated ? 'var(--wc-am)' : 'var(--wc-t2)' }}>
                    {t.km > 0 ? `${t.km} km` : t.autoGenerated ? '? km' : `${t.km} km`}
                  </div>
                  <span
                    className="font-heading font-bold text-[11px] px-2 py-[3px] rounded-[6px] uppercase tracking-[.04em] flex-shrink-0"
                    style={t.autoGenerated ? {
                  ...
                  >
                    {t.autoGenerated ? 'Connector' : t.type === 'business' ? 'Business' : 'Personal'}
                  </span>
                ...
                {isExpanded && (
                  <div className="p-[10px_14px_13px] flex flex-col gap-2 border-t" style={{ borderColor: t.autoGenerated ? 'rgba(245,158,11,.15)' : 'var(--wc-border)' }}>
                    {t.autoGenerated ? (
                      <div className="text-[11px] leading-[1.6]" style={{ color: 'var(--wc-t3)' }}>
                        ...
                        <strong style={{ color: 'var(--wc-am)' }}>Distance:</strong> {t.km > 0 ? `${t.km} km` : 'Unknown — edit to set'}<br />
                        <strong style={{ color: 'var(--wc-am)' }}>Odometer:</strong> {oStart.toLocaleString('en-AU')} &rarr; {oEnd.toLocaleString('en-AU')} km
                      </div>
                    ) : (
